...
Die vorliegende Arbeit baut auf der von Friesen~\cite{friesen2021}
entwickelten Kommunikationsanbindung zwischen einem externen PC und
der KUKA KR~C4 auf. Für die Kommunikation werden daher ebenfalls
Ethernet als physikalische Verbindung und \ac{kvp} als
Kommunikationsschnittstelle verwendet. Eine alternative
Kommunikationstechnologie wird im Rahmen dieser Arbeit nicht betrachtet,
da die bestehende Implementierung und deren Erweiterung im Mittelpunkt
stehen.

Friesen~\cite{friesen2021} implementierte eine Anbindung zwischen
MATLAB und der KR~C4 über \ac{kvp}. Die KR~C4 besitzt neben der
echtzeitfähigen VxWorks-Seite eine eingebettete Windows-Umgebung, auf
der der \ac{kvp}-Serverdienst läuft. Die Kommunikation zwischen der
Windows-Umgebung und der VxWorks-Seite erfolgt intern über die von KUKA
bereitgestellte CrossComm-Schnittstelle. \ac{kvp} nimmt über
\ac{tcpip} byte-codierte Lese- und Schreibanfragen auf
\ac{krl}-Variablen entgegen und beantwortet diese mit den
entsprechenden Werten (Abbildung~\ref{fig:kvp_datenfluss}).
...
Bewegungssteuerungs- und Rückgabewert-Anforderungen als erfüllt ein.
Das
...
Projekts durchgeführten Baseline-Messung (Abschnitt~\ref{sec:baseline}).
Bei
...
\SI{140}{\milli\second}.
Dies ist eine deutliche, unvorhersehbare Streuung
...
(Kap.~3.2) grundsätzlich hydrostatische sowie elektromagnetische und
...
Laut Aufgabenstellung wurde die externe Sensorik in
...
Für die Umsetzung des Projekts wird \textbf{QUARC} (Quanser) als Echtzeit-Zielsystem für Simulink eingesetzt.
Der Ablauf entspricht dabei dem bereits aus den Laborversuchen dieses
Moduls bekannten Workflow.
Das Simulink-Blockschaltbild wird über den
...
Simulink-Blockdiagrammebene um eine zustandsbasierte Beschreibung. 
Zustände
...
Im Rahmen eines Zwischentests zur Totzeitkompensation wurde eine
Modellvariante zusätzlich um einen Smith-Prädiktor erweitert.

Der Smith-Prädiktor verwendet ein internes Streckenmodell, um den Einfluss
einer vorhandenen Totzeit auf die Regelung zu reduzieren. Hierzu wird das
Streckenverhalten ohne und mit nachgebildeter Totzeit betrachtet und daraus
ein korrigiertes Rückführsignal für den Regler gebildet
\cite{NormeyRico2022}. Das grundlegende Prinzip ist in
Abbildung~\ref{fig:smith} dargestellt.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \draw[conn] (-1.6,0) -- (s1)
        node[midway, above, font=\tiny]{Sollwert};

        \draw[conn] (s1) -- (reg);
        \draw[conn] (reg) -- (strecke);
        \draw[conn] (reg.south) -- (model.north);

        \draw[conn] (model) -- (delay)
        node[midway, above, font=\tiny]{$\hat{z}_{\text{ohne}}$};

        \draw[conn] (model.west) -- (spred.east)
        node[midway, above, font=\tiny]{$+$};

        \draw[conn] (delay.south) -- (5.2,-2.3) -- (0,-2.3) -- (spred.south)
        node[near start, below, font=\tiny]{$-\ \hat{z}_{\text{mit}}$};

        \draw[conn] (strecke.east) -- (7.0,0) -- (7.0,-3.2) -- (-0.35,-3.2) -- (spred.west)
        node[near start, right, font=\tiny]{$-$\ Sensor$_{\text{real}}$};

        \draw[conn] (spred.north) -- (s1.south)
        node[midway, right, font=\tiny]{Sensor$_{\text{pred}}$};
    \end{tikzpicture}

    \caption{Grundstruktur eines Smith-Prädiktors zur Totzeitkompensation
    in Anlehnung an \cite{NormeyRico2022}.}
    \label{fig:smith}
\end{figure}

Die konkrete Umsetzung des im Zwischentest verwendeten Smith-Prädiktors
wird in Abschnitt~4.5.4 beschrieben. Die Ergebnisse der Versuche werden
