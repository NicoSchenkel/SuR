...
\ac{tcpip}-Aussetzer hin.
Die ist ein Effekt, der auch mit einer deterministischen
...
Ergänzend zur MATLAB/KVP-Baseline (Abschnitt~\ref{sec:baseline}) wurde das
zeitliche Verhalten der neuen QUARC/Simulink-Anbindung untersucht.

Hierzu wurde zunächst dieselbe Fünf-Punkt-Wegpunktbahn ohne Sensorregelung
verwendet. Dadurch konnte überprüft werden, ob die neue QUARC-basierte
Anbindung die Bahnkommandos funktional reproduzieren kann, ohne dass
Sensorik oder Regelung die Untersuchung beeinflussen. Gleichzeitig wurde
die Zuverlässigkeit des QUARC-Modelltakts untersucht.

Das verwendete Simulink-Modell sendet die Bahnsollwerte PC-seitig deterministisch über den
QUARC-Echtzeitkern, wartet jedoch in dieser Ausbaustufe nicht auf eine
Rückmeldung des \ac{krl}-Programms aus der Steuerung (Pollen).
...
Analog zur Baseline-Messung wurden dazu 5 identische Messläufe durchgeführt. Ausgewertet wurden die
...
Ein generierter \ac{krl}-Befehl kann dabei folgendermaßen aussehen:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{images/quanserTime.png}
...
vom Soll ab und kein einziger der 1152 Samples überschreitet
...
taktet also durchgehend gleich.
...
Zur Bewertung des Regelungs- und Sensorverhaltens wurde ein Versuch mit dem
realen Ultraschallsensor durchgeführt. Dabei wurde der Distanz-Sollwert
(\texttt{soll}) bei $t=\SI{10.8}{\second}$ von
\SI{30}{\centi\meter} auf \SI{20}{\centi\meter} reduziert.
Parallel wurden der Positionsbefehl (\texttt{Target}), die tatsächliche
Roboterposition (\texttt{istPosition}), der Reglerausgang und das
Sensorsignal aufgezeichnet. Abbildung~\ref{fig:realsensor_einzeln} zeigt die
aufgenommenen Signalverläufe.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\linewidth]{images/realsensor_signale_einzeln.png}
    \caption{Signalverläufe beim Regelungsversuch mit realem Ultraschallsensor.}
    \label{fig:realsensor_einzeln}
\end{figure}

Vor der Sollwertänderung liegen \texttt{Target} und \texttt{istPosition}
nahezu deckungsgleich bei \SI{615.0}{\milli\meter}. Nach der Änderung erreicht
\texttt{Target} zunächst ein Minimum von \SI{427.7}{\milli\meter}, während
\texttt{istPosition} zeitlich verzögert auf ein Minimum von
\SI{436.6}{\milli\meter} folgt. Anschließend nähern sich beide Signale einem
stationären Wert von etwa \SI{506.9}{\milli\meter} an. Dabei zeigt sich ein
deutliches Überschwingen der Roboterbewegung.

Der sichtbare Zeitversatz zwischen \texttt{Target} und
\texttt{istPosition} verdeutlicht die Verzögerung des Systems. Die
Totzeit wurde unabhängig davon im Sprungversuch in Abschnitt~4.5.2
experimentell bestimmt.

Der Reglerausgang erreicht unmittelbar nach der Sollwertänderung zunächst
seinen unteren Grenzwert von $-80$. Nach der ersten Korrekturbewegung ändert
das Stellsignal sein Vorzeichen und nähert sich anschließend wieder einem
Wert nahe Null. Dies zeigt, dass die Begrenzung des Stellsignals während der
starken anfänglichen Korrektur aktiv wird.

Das Sensorsignal nähert sich nach der Sollwertänderung dem vorgegebenen
Abstand von \SI{20}{\centi\meter} an. Im stationären Zustand verbleibt nur
eine Abweichung von etwa \SI{0.1}{\centi\meter}. Im realen Betrieb traten
jedoch Schwankungen des Sensorsignals auf, die zu kleinen
Ausgleichsbewegungen des Endeffektors führten. Aus diesem Grund wurde in der
Regelung eine Dead Zone verwendet, damit kleine Messwertschwankungen nicht
direkt in neue Bewegungsbefehle umgesetzt werden.

Der Versuch zeigt damit, dass der vorgegebene Abstand grundsätzlich erreicht
wird, während Totzeit, Begrenzung des Stellsignals und Schwankungen des
Ultraschallsensors das dynamische Regelverhalten beeinflussen.
Ergänzend wurde die in Abschnitt~4.5.4 beschriebene Smith-Kompensation
praktisch untersucht. Ausgangspunkt war eine Wartezeit von 30 Ticks zwischen
zwei Positionsaktualisierungen. Mit aktivierter Smith-Kompensation wurde
anschließend eine reduzierte Wartezeit von 20 Ticks getestet. Bei einer
weiteren Reduktion auf 10 Ticks kam es dagegen zu starken Schwingungen,
sodass der Versuch aus Sicherheitsgründen abgebrochen wurde.
Abbildung~\ref{fig:smith_20ticks} zeigt die Signalverläufe des Versuchs
mit 20 Ticks.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{images/smith_20ticks.jpg}
    \caption{Signalverläufe beim Regelungsversuch mit aktivierter
    Smith-Kompensation und einer Wartezeit von 20 Ticks zwischen den
    Positionsaktualisierungen.}
    \label{fig:smith_20ticks}
\end{figure}

Die Signalverläufe zeigen, dass während der Positionsänderung die
Smith-Korrektur aktiv wird und das System anschließend wieder einen
stationären Zustand erreicht. Im durchgeführten Versuch trat bei einer
Wartezeit von 20 Ticks kein instabiles Verhalten auf.

Eine weitere systematische Untersuchung war anschließend nicht mehr möglich,
da der Ultraschallsensor keine zuverlässigen Messwerte mehr lieferte.
Daher lässt sich aus den durchgeführten Versuchen keine robuste Verbesserung
des Gesamtsystems durch die Smith-Kompensation ableiten. Die
Smith-Kompensation wurde nicht in die final verwendete Regelungsversion
übernommen.